...
# Project-local build (vendored). Credentials come from the environment or the
# project .env (DATAFORSEO_LOGIN / DATAFORSEO_PASSWORD); the home-folder config
# file is only a fallback. Outputs go to <project>/tools/dataforseo/outputs.
PROJECT_ROOT = Path(__file__).resolve().parents[4]
CONFIG_PATH = Path(os.environ.get("DATAFORSEO_CONFIG", str(Path.home() / ".dataforseo_config.json")))
OUTPUT_DIR = Path(os.environ.get("DATAFORSEO_OUTPUT_DIR", str(PROJECT_ROOT / "tools" / "dataforseo" / "outputs")))
...
def load_credentials() -> tuple[str, str]:
    """Load credentials: env vars, then the project .env, then the config file."""
    env = {**_read_project_env(), **{k: v for k, v in os.environ.items() if k.startswith("DATAFORSEO_")}}
    login, password = env.get("DATAFORSEO_LOGIN"), env.get("DATAFORSEO_PASSWORD")
    if login and password:
        return login, password
    if not CONFIG_PATH.exists():
        raise FileNotFoundError(
            "Credentials not found. Add DATAFORSEO_LOGIN and DATAFORSEO_PASSWORD to the project .env"
        )
